Scriptname MintyProjectileStormScript extends activemagiceffect  
...
Bool property bMissOnPurpose = True auto ; Minty changed from false
			...
			if (ActivatorRef.GetParentCell() != none)
				if SpellRef != none
					...
					if bMissOnPurpose == True
						...
						int randomAmountOfCasts = Utility.RandomInt(0, 10)
						int loopCount = 0
						while (loopCount < randomAmountOfCasts)
							SpellRef.RemoteCast(ActivatorRef,CasterActor,ActivatorTargetRef) ; Minty TEST
							loopCount += 1
						endWhile
